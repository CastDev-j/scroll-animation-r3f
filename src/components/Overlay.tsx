...
const Section = ({ children, right, opacity}: SectionProps) => {
  return (
    <section
      className={cn(
        "h-screen w-screen flex flex-col justify-center p-10",
        right ? "items-start" : "items-end",
      )}
      style={{ opacity: opacity ?? 1 }}
    >
      <div className="max-w-sm w-full">
        <div className="bg-[#fefefe] shadow-xl border border-gray-200 rounded-2xl px-4 py-6">
          {children}
        </div>
      </div>
    </section>
    ...
    <Scroll html>
      <Section right opacity={opacityFirstSection}>
        ...
        <div className="mb-3 text-sm text-gray-600">
          Me especializo en desarrollo front-end, aprendiendo y aplicando
          conceptos en proyectos personales. Aunque aún no tengo experiencia
          formal, me esfuerzo por mejorar constantemente mis habilidades.
        </div>
      ...
      <Section opacity={opacitySecondSection}>
        <div className="font-serif text-2xl text-gray-800 mb-4">Tech Stack</div>
